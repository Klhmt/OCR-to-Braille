...
if __name__=="__main__" :
    ...
    nom_image = 'Test_folder\regions\region1\region1.jpg'
    # plt.figure()
    io.imshow(nom_image)
    # plt.show()
    # plt.figure()
    # plt.imshow(f'{source_folder}/{region}/{image_region[0]}')
    # plt.show()
    ...
    indices_lignes = separe_en_lignes(nom_image)
    # indices_debut_fin_ligne = indices_lignes[0]

    #Sur cette prrmière ligne on sépare les caractères
    print(indices_lignes)
    # ranges = separe_en_caracteres(nom_image, indices_debut_fin_ligne)

    # for elt in ranges :
    #     plt.figure()
    #     plt.imshow(nom_image[indices_debut_fin_ligne[0]:indices_debut_fin_ligne[1], elt[0]:elt[1]])
    #     plt.show()
